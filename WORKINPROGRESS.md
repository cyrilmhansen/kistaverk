...
- Sensor logger: Rust screen now exposes toggles (accel/gyro/mag/pressure/GPS/battery), interval binding, status/error text, and share gating. Kotlin logger consumes bindings, registers only selected sensors at the chosen interval on a background thread, writes CSV to the user-visible Documents directory when possible (fallback to app-private) with FileProvider sharing, updates Rust status/path during logging, and requests location permission only when GPS is enabled.
- Sensor logger: Rust screen now exposes toggles (accel/gyro/mag/pressure/GPS/battery), interval binding, status/error text, and share gating. Kotlin logger consumes bindings, registers only selected sensors at the chosen interval on a background thread, writes CSV to the user-visible Documents directory when possible (fallback to app-private) with FileProvider sharing, throttles status UI updates to stay responsive, updates Rust status/path during logging, and requests location permission only when GPS is enabled.
