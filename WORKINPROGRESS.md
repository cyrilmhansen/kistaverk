...
- Renderer guardrails: Kotlin validates JSON with a widget whitelist/required children before rendering; malformed payloads fall back to an inline error screen. Buttons must carry action or copy_text; ImageBase64/ColorSwatch enforce required data. Accessibility strings flow through `content_description`. Clipboard copy supported via `copy_text` on buttons; clipboard text is injected into bindings when small.
