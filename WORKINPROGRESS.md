...
## Snapshot
- Kotlin now launches the system file picker (detaching FDs), forwards a `bindings` map with UI state, and renders Text/Button/ShaderToy/TextInput/Checkbox/Progress; Columns/Grids can opt out of ScrollView wrapping via `scrollable` (default true). Renderer validation enforces required fields (Button text + action/copy_text, TextInput/Checkbox bind_key, PdfPagePicker data) and falls back to an inline error screen. Overlay spinner keeps prior screen visible during loading-only calls.
...
- Features: streaming hashes (SHA-256/SHA-1/MD5/MD4/CRC32/BLAKE3), Shader demo, Kotlin image conversion flow with output dir selection, Text Tools (upper/lower/title/wrap/trim/count/Base64/URL/Hex) with copy/share hooks, Progress demo, File info, Text viewer (text/CSV preview with 256KB cap + Files-app intent), Archive viewer (ZIP listing with truncation notice), QR generator (base64 PNG), Color Converter (Hex↔RGB/HSL with swatch and per-format copy buttons), PDF tools (page picker with horizontal thumbnail carousel + extract/delete/merge/title, signature stamping via lopdf with px→pt conversion, aspect-ratio-based default sizing, and Y-flip using MediaBox height, PdfRenderer thumbnails, SignaturePad capture + dimension/DPI reporting), About screen (version, copyright, GPLv3).
- Renderer guardrails: Kotlin validates JSON with a widget whitelist/required children before rendering; malformed payloads fall back to an inline error screen. Buttons must carry action or copy_text; ImageBase64/ColorSwatch enforce required data; PdfPagePicker validation is covered by tests. Accessibility strings flow through `content_description`. Clipboard copy supported via `copy_text` on buttons; clipboard text is injected into bindings when small.
...
- Sensor logger: Rust screen exposes toggles (accel/gyro/mag/pressure/GPS/battery), interval binding, status/error text, and share gating. Kotlin logger consumes bindings, registers only selected sensors at the chosen interval on a background thread, writes CSV to the user-visible Documents directory when possible (fallback to app-private) with FileProvider sharing, throttles status UI updates to stay responsive, guards late callbacks via a volatile flag so writers aren’t touched after stop, updates Rust status/path during logging, and requests location permission only when GPS is enabled.

## Known Issues / Risks
- Renderer still trusts incoming JSON beyond the basic whitelist; add stronger schema validation and richer error UI.
- Limited test coverage on dispatch/render; Robolectric tests are mocked to skip JNI and not yet wired into CI.
- Cargo build task targets arm64-v8a only; stale v7a artifacts may linger locally and should be cleaned; MediaStore/SAF UX still needs on-device validation.
- Gradle wrapper download may hit filesystem permission errors on some hosts; keep ~/.gradle writable or vendor the distribution.
- Sensor logger still needs on-device validation for TalkBack labels, GPS permission flow, and CSV accuracy/throttling at high sample rates.
