...
- Build portability: Gradle cargo task now resolves `cargo` from PATH/`CARGO` env (no hardcoded /usr/bin), but Cargo.lock is still stale; armv7 build still invoked.
