...
- **Launcher alias (planned):** An `activity-alias` will expose a “Sign PDF” icon pointing to `MainActivity` with an intent extra to start on the PDF signature screen, keeping a single APK with two entry points.
