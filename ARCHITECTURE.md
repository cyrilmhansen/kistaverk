...
- **Schema guardrails:** Renderer validates incoming JSON (widget whitelist + required children for Column/Grid + required fields for Button/TextInput/Checkbox/ImageBase64/ColorSwatch/PdfPagePicker) before rendering and falls back to an inline error screen if malformed.
...
- **Dependency listing:** A build-time script (`rust/scripts/generate_deps_metadata.sh`) writes `deps.json` from `cargo metadata` into Android assets; `DepsList` widget renders it in a scrollable list inside About (tested with a stubbed asset to ensure rendering works when the JSON is present).
